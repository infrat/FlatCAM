...
> **macOS Apple Silicon fork** – This fork focuses on porting FlatCAM to run
> natively on macOS with Apple Silicon (arm64). It includes a CI pipeline that
> builds a universal `.app` bundle and `.dmg` installer via PyInstaller on
> GitHub Actions. See the [Releases](../../releases) page for ready-to-use
> macOS packages.
...
### Pre-built .app / .dmg (recommended)

1. Go to the [Releases](../../releases) page.
2. Download **FlatCAM.dmg** (or **FlatCAM-app.zip**).
3. Open the `.dmg` and drag **FlatCAM.app** into your **Applications** folder.
4. On first launch macOS Gatekeeper may block the app. Right-click the app → **Open** → confirm, or run:
   ```
   xattr -cr /Applications/FlatCAM.app
   ```
5. Launch FlatCAM from Applications.
...
git clone https://github.com/<your-fork>/FlatCAM.git
cd FlatCAM
...
pip install -r FlatCAM/requirements.txt
pip install pyinstaller language_data

# Compile Qt resources
pyrcc5 FlatCAM/resources.qrc -o FlatCAM/resources.py
pyrcc5 FlatCAM/resources_dark.qrc -o FlatCAM/resources_dark.py

# Run from source
cd FlatCAM
python FlatCAM.py

# (Optional) Build a standalone .app bundle
cd ..
pyinstaller --clean -y flatcam.spec
# The app is now at dist/FlatCAM.app
```
