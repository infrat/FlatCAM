	...
	def on_toggle_all_rows(self):
		...
		if len(sel_rows) == self.ui.tools_table.rowCount():
			...
			self.ui.tool_data_label.setText(
				f'<b>{_("Parameters for")}: <font color="{get_fg_blue()}">{_("No Tool Selected")}</font></b>')
		else:
			self.ui.tools_table.selectAll()
			self.ui.tool_data_label.setText(
				f'<b>{_("Parameters for")}: <font color="{get_fg_blue()}">{_("Multiple Tools")}</font></b>')
	...
	def update_ui(self):
		...
		for current_row in sel_rows:
			...
			if len(sel_rows) == 1:
				cr = current_row + 1
				self.ui.tool_data_label.setText(
					f'<b>{_("Parameters for")}: <font color="{get_fg_blue()}">{_("Tool")} {cr}</font></b>')
				try:
...
class NccUI:
	...
	def __init__(self, layout, app):
		...
		self.tool_data_label = FCLabel(
			f'<b>{_("Parameters for")}: <font color="{get_fg_blue()}">{_("Tool")} 1</font></b>')
