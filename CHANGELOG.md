...
- move versioning to calendar basis, now version is 2023.5. Change beta naming to greek letter
- Python 3.10 required
- fixes bugs so now almost all tools works. But some of command-line options not working
- fixes colors bugs, now dark theme is default
- version check and sending statictics is off by default
- fixed <Ctrl+C> keyboard shortcut in comman-line to quit
- add 'Components' tab to 'About' dialog
- add command-line argument '--version'
- move development process to pyenv and poetry
...
- moved the init of activity view in the MainGUI file from the APP.**init**()
...
- fixed a small issue in Gerber file opener filter that did not see the _.TOP extension or _.outline extension
...
- fixed a bug in extension detection for Gerber files that allowed in the filtered list files that extension _.gb_
...
- moved app.setup_obj_classes() upper in the App.**init**()
...
- replaced in FlatCAM Tools and in FLatCAMObj.py and in Editors all references to hardcoded decimals in string formats for tools with a variable declared in the **init**()
